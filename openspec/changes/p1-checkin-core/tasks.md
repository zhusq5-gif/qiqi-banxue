...
- [x] 3.1 实现注册/登录/退出界面与会话保持（登录后刷新页面不掉线），验证注册（含邮箱验证码）→登录→退出→未登录访问被拒全流程（线上 E2E 全通过：注册验证码两步流/自动登录/刷新保持/退出清除会话/重新登录，2026-08-28）
- [x] 3.2 实现儿童档案创建引导页（昵称必填校验 + 预设 SVG 头像选择 + 生日），验证首次登录进入引导（空昵称为前端必填拦截，代码路径明确；线上 E2E 引导流程实测通过，2026-08-28）
- [x] 3.3 实现档案编辑页（昵称/头像/生日修改），验证修改后全应用展示同步更新（线上 E2E：改名「小七七」保存成功并全局生效，2026-08-28）
- [x] 3.4 制作预设 SVG 头像与科目图标库（`src/content/` 清单文件），验证头像/图标可被选择并正确渲染（8 头像 + 12 科目图标线上渲染与选择实测通过，2026-08-28）
