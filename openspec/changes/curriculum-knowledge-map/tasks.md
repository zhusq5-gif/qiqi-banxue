...
- [x] 接入 2022 课标证据实体：2 份标准文档、17 条教育部来源证据条款、24 条候选映射
...
- [ ] 完成至少 1 名真实学科审核者的线下身份核验与公钥登记，形成第一条真实签名映射审批
...
1. `npm run build` 通过。
2. `npm test` 通过现有回归、课程种子不变量、课标证据、审核包与研究快照安全测试。
3. 登录后 `/knowledge-map`、`/knowledge-map/review`、`/knowledge-map/standards`、`/knowledge-map/standards/review` 可访问且不会修改儿童云端数据。
...
14. 正式发布门禁未满足时，不提供“正式发布成功”的误导性状态。
