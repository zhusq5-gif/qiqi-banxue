...
describe('AI discovery subject-grade search coverage', () => {
  ...
  it('reflects Wave06 depth additions without mistaking them for knowledge completeness', () => {
    ...
    expect(aiDiscoveryCoverageFor('chinese', 6)?.candidateCount).toBe(12)
    ...
    expect(aiDiscoveryCoverageFor('english', 5)?.candidateCount).toBe(9)
    expect(aiDiscoveryCoverageFor('english', 6)?.candidateCount).toBe(9)
