...
describe('wave 2 math verification', () => {
  it('surfaces grade 4 and 5 audit findings', () => {
  ...
  it('keeps math item screening separate from audit tasks', () => {
    ...
    expect(mathWave2AuditTasks.some((task) => task.kind === 'assessment_without_target' && task.severity === 'blocking')).toBe(true)
    expect(mathWave2AuditTasks.some((task) => task.kind === 'cross_grade_relation_review' && task.severity === 'review')).toBe(true)
  })

  it('adds assessment binding to the release gate', () => {
    ...
    expect(curriculumVerificationSummaryWave2.mathWave2BlockingTaskCount).toBeGreaterThanOrEqual(2)
